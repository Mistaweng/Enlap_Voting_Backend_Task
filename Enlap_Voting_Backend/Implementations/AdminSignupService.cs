...
{
	public class AdminSignupService : IAdminSignupService
	{
		private readonly UserManager<AppUser> _userManager;

		public AdminSignupService(UserManager<AppUser> userManager)
		{
			_userManager = userManager;
		}

		public async Task<ApiResponse> SignUpAsync(AdminSignupDto adminSignupDto)
		{
			// Check if the email is already associated with an existing user
			var existingUser = await _userManager.FindByEmailAsync(adminSignupDto.Email);
			...
			string generatedUsername = await GenerateUniqueUsernameAsync(
				adminSignupDto.Firstname, adminSignupDto.Lastname);

			// Create a new user with the generated username
			var user = new AppUser()
			{
				FirstName = adminSignupDto.Firstname,
				LastName = adminSignupDto.Lastname,
				Email = adminSignupDto.Email.Trim(),
				UserName = generatedUsername.Trim(),
				Password = adminSignupDto.Password.Trim(),
			...
			using (var transaction = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
			{
				...
				var createUser = await _userManager.CreateAsync(user, adminSignupDto.Password);
