...
namespace Enlap_Voting_Backend.Implementations
{
    ...
    public class ContestantSignupService : IContestantSignupService
    {
        private readonly UserManager<AppUser> _userManager;

        public ContestantSignupService(UserManager<AppUser> userManager)
        {
            _userManager = userManager;
        }


        public async Task<ApiResponse> SignUpAsync(ContestantSignupDto contestantSignupDto)
        {
            // Check if the email is already associated with an existing user
            var existingUser = await _userManager.FindByEmailAsync(contestantSignupDto.Email);
            ...
            string generatedUsername = await GenerateUniqueUsernameAsync(
                contestantSignupDto.Firstname, contestantSignupDto.Lastname);

            // Create a new user with the generated username
            var user = new AppUser()
            {
                FirstName = contestantSignupDto.Firstname,
                LastName = contestantSignupDto.Lastname,
                Email = contestantSignupDto.Email.Trim(),
                UserName = generatedUsername.Trim(),
                Password = contestantSignupDto.Password.Trim(),
                EmailConfirmed = false,
                UserRole = UserRoles.Contestant,
                PhoneNumber = contestantSignupDto.PhoneNumber.Trim()
            ...
            using (var transaction = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
            {
                ...
                var createUser = await _userManager.CreateAsync(user, contestantSignupDto.Password);
